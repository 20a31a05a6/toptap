...
class ProfileScreenController extends BlockUserController
  ...
  RxList<Post> favoritePosts = <Post>[].obs;
  ...
  Future<void> fetchLocked({bool isEmpty = false}) async {
    ...
    try {
      UserPostData? items =
          await PostService.instance.fetchUserPosts(
        type: PostType.posts, // temporarily use 'posts'
        //  filter:
        //     'locked', // if your backend supports a 'filter'
        userId: userData.value?.id?.toInt(),
        lastItemId: isEmpty
            ? null
  ...
  Future<void> fetchFavorites(
    ...
    try {
      UserPostData? items =
        ...
        lastItemId: isEmpty
            ? null
            : favoritePosts.lastOrNull?.id?.toInt(),
      );
      if (isEmpty) favoritePosts.clear();

      if (favoritePosts.isEmpty) {
        favoritePosts.addAll(items?.pinnedPostList ?? []);
      }
  ...
  Future<void> fetchSaved({bool isEmpty = false}) async {
    ...
    try {
      UserPostData? items =
          await PostService.instance.fetchUserPosts(
        type: PostType
            .posts, // Again, backend should support this
        userId: userData.value?.id?.toInt(),
        lastItemId: isEmpty
            ? null
            : savedPosts.lastOrNull?.id?.toInt(),
      );
      if (isEmpty) savedPosts.clear();

      if (savedPosts.isEmpty) {
        savedPosts.addAll(items?.pinnedPostList ?? []);
      }
