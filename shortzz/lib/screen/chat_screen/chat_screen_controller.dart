...
class ChatScreenController extends BlockUserController
  ...
  void _getChat() async {
    ...
    var subscription = chatCollection
        .where(FirebaseConst.noDeleteIds,
            arrayContains: myUser?.id)
      ...
      chatList
          .sort((a, b) => b.id?.compareTo(a.id ?? 0) ?? 0);
  ...
  Future<void> fetchMoreChatList() async {
    ...
    try {
      var query = chatCollection
          ...
          .where(FirebaseConst.id,
              isGreaterThan:
                  conversationUser.value.deletedId)
          .orderBy(FirebaseConst.id, descending: true)
          .limit(AppRes.chatPaginationLimit);
  ...
  // adding auiod call and voice call

  void startVoiceCall(AppUser? user) {
    if (user == null) return;
    // TODO: Implement actual voice call logic here
    Loggers.success(
        'Starting voice call with ${user.username}');
    showSnackBar('Voice call feature coming soon!');
  }
  ...
  void startVideoCall(AppUser? user) {
    ...
    showSnackBar('Video call feature coming soon!');
  }
